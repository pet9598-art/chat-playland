...
- **관리자 모드**: 로비 화면 하단의 "🔐 관리자 모드"에서 비밀번호를 확인하면, 지금 열려있는 모든 방을 한 번에 정리(삭제)할 수 있어요(수업 종료 후 정리용). 기본 비밀번호는 `hurian2026`이며, `app.js`의 `ADMIN_PASSWORD_HASH` 주석을 참고해 바꿀 수 있습니다.
